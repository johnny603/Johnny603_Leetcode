/*
- Sort the input paths by their depth (length), and lexicographically for consistency
- Build a Trie where each node represents a folder in the hierarchy
- Traverse the Trie to serialize each subtree into a unique string format
  - For each node, concatenate the folder name with the serialization of its children (sorted by name)
  - Store the serialized subtree string in a hash map that maps to all nodes with the same subtree
- Mark all nodes that have duplicate serialized subtrees (i.e., appear more than once in the map)
- Traverse the Trie again to construct the remaining valid folder paths
  - Skip any paths that pass through a deleted (duplicate) subtree
  - Collect and return all valid paths
*/
