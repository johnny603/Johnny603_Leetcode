...
class Solution {
    ...
    public int[] maxTargetNodes(int[][] edges1, int[][] edges2) {
        ...
        // Find the best you can do by adding the extra edge in tree-2
        ...
        // Run BFS on tree-1, and record nodes on even levels
        boolean[] included = new boolean[n1];
        int even_count1 = bfs(0, adj1, included);

        // For each i, if i was even-level in tree-1 we connect it to tree-2's best even-level
        int[] ans = new int[n1];
        for (int i = 0; i < n1; ++i) {
            if (included[i]) {
